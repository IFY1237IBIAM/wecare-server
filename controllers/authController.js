...
exports.signup = async (req, res) => {
  try {
    ...
    if (existingPseudonym) return res.status(409).json({ message: "That pseudonym is already taken. Try another." });
...
exports.login = async (req, res) => {
  try {
    ...
    return res.json({
      ...
      token,
      twoStep:     userWithSecurity.twoStepEnabled  || false,
      twoStepHint: userWithSecurity.twoStepHint     || "",
      user: {
        id:                  user._id,
        _id:                 user._id,
        pseudonym:           user.pseudonym,
...
exports.searchUsers = async (req, res) => {
  try {
    ...
    const users = await User.find({
      ...
      isBanned:  { $ne: true },
    }).select("pseudonym avatar isOnline lastSeen showOnlineStatus createdAt").limit(20);
