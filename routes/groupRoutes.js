...
// // GET /api/groups/:groupId/members — enhanced with email for Circle_Keeper check
// router.get("/:groupId/members", protect, requireMember, async (req, res) => {
//   try {
//     const group = await Group.findById(req.params.groupId)
//       .populate("members", "pseudonym email");
//     const members = group.members.map((m) => ({
//       _id: m._id,
//       pseudonym: m.pseudonym,
//       email: m.email,
...
//     }));
//     return res.json({ members, creatorId: group.creator, isClosed: group.isClosed || false });
//   } catch (e) {
//     return res.status(500).json({ message: e.message });
//   }
// });
...
//   try {
//     const group = await Group.findById(req.params.groupId);
//     const requestingUser = await require("../models/User").findById(req.user._id);

//     const isCreator = group.creator.toString() === req.user._id.toString();
//     const isAdminKeeper = requestingUser.email === "mom@gmail.com";

//     if (!isCreator && !isAdminKeeper) {
...
//     // Add to mutedMembers array (add field to Group model or use a separate collection)
//     if (!group.mutedMembers) group.mutedMembers = [];
//     const alreadyMuted = group.mutedMembers.some((m) => m.toString() === req.params.userId);
//     if (!alreadyMuted) {
...
// // DELETE /api/groups/:groupId/members/:userId — remove member, Circle_Keeper only
...
//     // Socket notify the removed user
//     if (req.io) {
...
// // POST /api/groups/:groupId/close — close group, Circle_Keeper only
...
//     group.isClosed = !group.isClosed; // toggle
//     await group.save();
...
// ── GET members (FINAL SINGLE VERSION) ─────────────────────────────
router.get("/:groupId/members", protect, requireMember, async (req, res) => {
  try {
    const group = await Group.findById(req.params.groupId)
      .populate("members", "pseudonym email");
  ...
  try {
    ...
    if (group.isClosed) {
      return res.status(403).json({
        message: "This circle is currently closed",
      });
    }
    ...
    if (isMuted) {
      return res.status(403).json({
        message: "You are muted in this circle",
      });
    }
...
router.post("/:groupId/close", protect, requireMember, async (req, res) => {
  try {
    ...
    const isCreator = group.creator.toString() === req.user._id.toString();
    const isAdminKeeper = user.email === "mom@gmail.com";

    if (!isCreator && !isAdminKeeper) {
      return res.status(403).json({
        message: "Only the Circle_Keeper can close this circle",
      });
    }
